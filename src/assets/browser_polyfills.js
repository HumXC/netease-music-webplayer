(() => {
        ...
        document.addEventListener(
            ...
            function (event) {
                if (event.target instanceof HTMLMediaElement)
                    globalThis.__neteaseActiveMedia = event.target;
            },
            true,
        );
    }
    ...
    function controlNodes(root) {
        return Array.from(
            (root || document).querySelectorAll(
                'button, [role="button"], a, [title], [aria-label]',
            ),
        ).filter(visibleNode);
    }
    ...
    function findLikeControl() {
        ...
        const icon = root.querySelector(
            '.cmd-icon-like[aria-label="like"][title="喜欢"], ' +
                '.cmd-icon-like[aria-label="like"][title="取消喜欢"]',
        );
    ...
    function pendingLikeState() {
        const pending = globalThis.__neteasePendingLike;

        if (!pending) return null;

        if (Date.now() > pending.expiresAt) {
            delete globalThis.__neteasePendingLike;
            return null;
        }
    ...
    function setPendingLikeState(value) {
        globalThis.__neteasePendingLike = {
            value: !!value,
            expiresAt: Date.now() + 2000,
        };
    }

    function resolveLikeState(found) {
        const actual = activeLikeState(found);
        const pending = pendingLikeState();
        ...
        /*
         * Once the DOM catches up with our optimistic state,
         * pending is no longer needed.
         */
        if (actual === pending) {
            delete globalThis.__neteasePendingLike;
            return actual;
        ...
        /*
         * The DOM still exposes the previous state.
         * Keep reporting the expected state for a short grace period.
         */
        return pending;
    }
    ...
    function clickLikeControl(found) {
        ...
        /*
         * Set this before click() because Netease may synchronously cause
         * MutationObserver / other metadata refresh callbacks.
         */
        setPendingLikeState(nextLiked);

        /*
         * One logical click only.
         *
         * Do NOT separately dispatch events to both the icon and button.
         * The icon is inside the button, so normal event propagation already
         * reaches everything that should receive the click.
         */
        found.node.click();
    ...
    function findRepeatControl() {
        ...
        if (!root) return null;
        const icon = root.querySelector(
            '.cmd-icon[title="随机播放"], .cmd-icon[title="顺序播放"], .cmd-icon[title="心动模式"], .cmd-icon[title="列表循环"], .cmd-icon[title="单曲循环"]',
        );
        if (!icon || !visibleNode(icon)) return null;
        const button = icon.closest("button");
        if (!button || !visibleNode(button)) return null;
    ...
    function findMuteControl() {
        ...
        if (!root) return null;
        const icon = root.querySelector(
            '.right-side .cmd-icon-volume[aria-label="volume"][title="静音"], .right-side .cmd-icon-mute[aria-label="mute"][title="解除静音"]',
        );
        if (!icon) return null;
        const button = icon.closest("button");
    ...
    function collectNowPlayingFromThisFrame() {
        ...
        let title =
            ...
            textOf('a[href*="/song"], [class*="song"], [class*="name"], [class*="title"]') ||
            attrOf("[title]", "title");
        let artist =
            cleanText(meta.artist) ||
            textOf('a[href*="/artist"], [class*="artist"], [class*="author"], [class*="singer"]');
        const album = cleanText(meta.album);
        const artwork =
            (meta.artwork && meta.artwork[0] && meta.artwork[0].src) || attrOf("img[src]", "src");
        const media = currentMedia();
        ...
        const liked = resolveLikeState(likeControl);
        const repeatMode = readRepeatMode(repeatControl);
        const muted = activeMutedState(muteControl);

        if (!title && play) {
            const bad =
                /播放|暂停|上一|下一|喜欢|收藏|列表|歌词|音量|循环|随机|prev|next|play|pause|volume/i;
            const texts = [];
            for (const node of Array.from(root.querySelectorAll("a, span, div, p")).filter(
                visibleNode,
            )) {
                const t = cleanText(node.textContent || node.getAttribute("title") || "");
                if (t.length >= 1 && t.length <= 80 && !bad.test(t) && !texts.includes(t))
                    texts.push(t);
    ...
    if (!globalThis.__neteaseNowPlayingPublisher) {
        ...
        document.addEventListener(
            "click",
            function () {
                setTimeout(publishNowPlaying, 100);
            },
            true,
        );
